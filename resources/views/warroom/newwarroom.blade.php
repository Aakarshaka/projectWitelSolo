...
<div class="main-content">
    <div class="container">
        <div class="header">
            <div>
                <h1>WAR ROOM ACTIVITY WITEL JATENG TIMUR BULAN JUNI 2025</h1>
                <p>Jumlah Action Plan</p>
            </div>
        </div>
                ...
                <table class="table">
                    <thead>
                        <tr>
                            ...
                            <th style="background: #22c55e; color: white;">Update Action Plan</th>
                            <th style="background: #22c55e; color: white;">Status Action Plan</th>
                            <th>Aksi</th>
                        </tr>
                    </thead>
                    <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="btn-group-vertical">
                                        <button type="button" class="btn btn-sm btn-warning" onclick="openModal('editModal{{ $item->id }}')">
            ...
            <div class="modal-content">
                <form action="{{ route('newwarroom.update', $item->id) }}" method="POST">
                    ...
                    <div class="modal-header bg-warning">
                        ...
                        <button type="button" class="modal-close" onclick="closeModal('editModal{{ $item->id }}')">&times;</button>
                    </div>
                    <div class="modal-body">
                        <div class="form-row">
                            <div class="form-group">
                                <label for="tgl_{{ $item->id }}" class="form-label">Tanggal <span class="required">*</span></label>
                                <input type="date" class="form-control" id="tgl_{{ $item->id }}" name="tgl" value="{{ $item->tgl }}" required>
                            </div>
                            <div class="form-group">
                                <label for="agenda_{{ $item->id }}" class="form-label">Agenda <span class="required">*</span></label>
                                <input type="text" class="form-control" id="agenda_{{ $item->id }}" name="agenda" value="{{ $item->agenda }}" required>
                            </div>
                        </div>
                        <div class="form-row">
                            <div class="form-group">
                                <label for="uic_{{ $item->id }}" class="form-label">UIC <span class="required">*</span></label>
                                <input type="text" class="form-control" id="uic_{{ $item->id }}" name="uic" value="{{ $item->uic }}" required>
                            </div>
                            <div class="form-group">
                                <label for="peserta_{{ $item->id }}" class="form-label">Peserta <span class="required">*</span></label>
                                <input type="text" class="form-control" id="peserta_{{ $item->id }}" name="peserta" value="{{ $item->peserta }}" required>
                            </div>
                        </div>
                        <div class="form-group">
                            <label for="pembahasan_{{ $item->id }}" class="form-label">Pembahasan <span class="required">*</span></label>
                            <textarea class="form-control" id="pembahasan_{{ $item->id }}" name="pembahasan" rows="3" required>{{ $item->pembahasan }}</textarea>
                        </div>
                        <div class="form-group">
                            <label for="action_plan_{{ $item->id }}" class="form-label">Action Plan <span class="required">*</span></label>
                            <textarea class="form-control" id="action_plan_{{ $item->id }}" name="action_plan" rows="3" required>{{ $item->action_plan }}</textarea>
                        </div>
                        <div class="form-group">
                            <label for="support_needed_{{ $item->id }}" class="form-label">Support Needed</label>
                            <textarea class="form-control" id="support_needed_{{ $item->id }}" name="support_needed" rows="3">{{ $item->support_needed }}</textarea>
                        </div>
                        <div class="form-group">
                            <label for="info_kompetitor_{{ $item->id }}" class="form-label">Info Kompetitor</label>
                            <textarea class="form-control" id="info_kompetitor_{{ $item->id }}" name="info_kompetitor" rows="3">{{ $item->info_kompetitor }}</textarea>
                        </div>
                        <div class="form-row">
                            <div class="form-group">
                                <label for="jumlah_action_plan_{{ $item->id }}" class="form-label">Jumlah Action Plan</label>
                                <input type="number" class="form-control" id="jumlah_action_plan_{{ $item->id }}" name="jumlah_action_plan" value="{{ $item->jumlah_action_plan }}" min="0">
                            </div>
                            <div class="form-group">
                                <label for="status_action_plan_{{ $item->id }}" class="form-label">Status Action Plan</label>
                                <select class="form-control" id="status_action_plan_{{ $item->id }}" name="status_action_plan">
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" id="update_action_plan_{{ $item->id }}" name="update_action_plan" rows="3">{{ $item->update_action_plan }}</textarea>
                        </div>
                    </div>
                ...
                <div class="modal-body">
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control" id="jumlah_action_plan" name="jumlah_action_plan" min="0" value="0">
                        </div>
                        <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control" id="update_action_plan" name="update_action_plan" rows="3"></textarea>
                    </div>
                </div>
                <div class="modal-footer">
